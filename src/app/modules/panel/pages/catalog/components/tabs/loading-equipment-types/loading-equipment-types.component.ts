...
@Component({
  selector: 'app-loading-equipment-types',
  templateUrl: './loading-equipment-types.component.html',
  styleUrls: ['./loading-equipment-types.component.scss'],
    providers: [DialogService]
})
export class LoadingEquipmentTypesComponent implements OnInit, OnDestroy {
    ...
    loadData() : void {
        this.catalogService.getLoadingEquipmentsPaginate(this.dataPaginate).subscribe((response) => {
                this.loading = false;
    ...
    addUpdate(type: string, item: any, info: boolean): void {
        ...
        const dialog = this.dialogService.open(CrudComponent, {
            data: {
                ...
                item: item,
                info: info,
                url: '/catalogs/loading-equipment-types/',
                input: 1
            },
            header: `${title}`,
            width: sizeModal,
