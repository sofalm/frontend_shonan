...
@Component({
  selector: 'app-manufacture-types',
  templateUrl: './manufacture-types.component.html',
  styleUrls: ['./manufacture-types.component.scss'],
    providers: [DialogService]
})
export class ManufactureTypesComponent implements OnInit, OnDestroy {
    ...
    loadData() : void {
        this.loading = true;
        this.catalogService.getManufacturesPaginate(this.dataPaginate).subscribe((response) => {
    ...
    addUpdate(type: string, item: any, info: boolean): void {
        ...
        const dialog = this.dialogService.open(CrudComponent, {
            data: {
                ...
                item: item,
                info: info,
                url: '/catalogs/manufacture-types/',
                input: 1
            },
            header: `${title}`,
            width: sizeModal,
