...
@Component({
  selector: 'app-work-height-types',
  templateUrl: './work-height-types.component.html',
  styleUrls: ['./work-height-types.component.scss'],
    providers: [DialogService]
})
export class WorkHeightTypesComponent implements OnInit, OnDestroy {
    ...
    loadData() : void {
        this.loading = true;
        this.catalogService.getWorkHeightsPaginate(this.dataPaginate).subscribe((response) => {
    ...
    addUpdate(type: string, item: any, info: boolean): void {
        ...
        const dialog = this.dialogService.open(CrudComponent, {
            data: {
                ...
                item: item,
                info: info,
                url: '/catalogs/work-height-types/',
                input: 1
            },
            header: `${title}`,
            width: sizeModal,
