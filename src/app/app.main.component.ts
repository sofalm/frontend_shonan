...
export class AppMainComponent implements AfterViewInit, OnDestroy {
    ...
    messageServiceCustom(message: string , title: string, data: string, icon: string) {
        this.messageService.add({severity: message, summary: title, detail: data , icon: icon, life: 5000});
    }
    ...
    errorDialog(error: any) {
        let sizeModal = '30%';
        if (this.isMobile()) sizeModal = '90%'
        this.dialogService.open(ModalErrorComponent, {
            data: error,
            header: `Error`,
            width: sizeModal,
